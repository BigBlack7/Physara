...
{
    return vec3(SanitizeHDRComponent(value.r), SanitizeHDRComponent(value.g), SanitizeHDRComponent(value.b));
}
...
void main()
{
    ...
    vec3 sceneColor = clamp(
        hdrColor * exposureCompensation * GetPreExposure(uFrame.camera),
        vec3(0.0),
        vec3(60000.0));
