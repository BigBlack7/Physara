...
void main()
{
    ...
    vec3 lower = vec3(0.0);
    lower += texture(uLowerBloom, inUV + vec2(offset, 0.0) * texel).rgb;
    lower += texture(uLowerBloom, inUV + vec2(-offset, 0.0) * texel).rgb;
    lower += texture(uLowerBloom, inUV + vec2(0.0, offset) * texel).rgb;
    lower += texture(uLowerBloom, inUV + vec2(0.0, -offset) * texel).rgb;
    lower += texture(uLowerBloom, inUV + vec2(offset, offset) * texel).rgb * 0.5;
    lower += texture(uLowerBloom, inUV + vec2(-offset, offset) * texel).rgb * 0.5;
    lower += texture(uLowerBloom, inUV + vec2(offset, -offset) * texel).rgb * 0.5;
    lower += texture(uLowerBloom, inUV + vec2(-offset, -offset) * texel).rgb * 0.5;
    lower *= 1.0 / 6.0;
    vec3 high = texture(uSceneColor, inUV).rgb;
    outColor = vec4(clamp(high + lower, vec3(0.0), vec3(60000.0)), 1.0);
}