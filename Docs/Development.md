...
- [√] P.1 建立手工性能基线：固定场景、相机、分辨率、渲染设置和构建配置；由人工运行或抓帧记录 CPU、GPU、各 pass 时间及硬件环境，作为后续对照基线。（电脑 B 轮次 0 已完成，见 `Docs/TaskDetail.md` P.1；电脑 A 预留）
- [√] P.2 完善并运行 GPU CPU↔GPU 契约检查：以 `Tools/verify_gpu_contracts.py` 校验结构布局、字段、SSBO 前缀、绑定及着色器声明；同步清理脚本和契约头中已失效的任务编号注释，防止契约漂移。（已加字段序列与 std140/std430 前缀硬检查并通过，见 `Docs/TaskDetail.md` P.2）
- [√] P.3 基于 P.1 的人工测量结果，复核阶段一中性能相关任务的实际优先级。（测量后攻击顺序见 `Docs/TaskDetail.md` P.3：阴影 5.1 最高优先；Bloom 8.1/8.2、提交合并 3.2/3.3 高优先；0.12 barrier、7.2/7.3 Cluster 下调）
...
- [ ] 3.4 修复 GBuffer 与 Forward 的 MDI 合并条件不一致：`GBufferPass::CanMerge` 的 bindless 分支仍要求 meshKey 相等，导致跨网格合并失效（P.1 实测 35 次 merge break、37 个直接提交，Deferred 提交成本显著高于 Forward）；同时收敛 GBuffer pass 开头的多次独立 clear。
